...
def move_steps_single_back(direction, steps):
    ...
    sleeptime = calculate_sleep_from_pps(read_speed_file())
    for x in range(steps):
        move_step_single(Pins.BACK_PUL.value, sleeptime)
    print('move finished')
...
def move_steps_single_front(direction, steps):
    ...
    sleeptime = calculate_sleep_from_pps(read_speed_file())
    for x in range(steps):
        move_step_single(Pins.FRONT_PUL.value, sleeptime)
    print('move finished')
...
def move_step_single(pin, sleeptime):
    GPIO.output(pin, 1)     # set pulse pin to 1/GPIO.HIGH/True
    sleep(sleeptime)
    GPIO.output(pin, 0)     # set pulse pin to 0/GPIO.LOW/False
    sleep(sleeptime)
